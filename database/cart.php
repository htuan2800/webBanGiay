...
class cart
{
    ...
    {
        $sql = "SELECT products.idProduct, imageproducts.image, products.productName, cartdetail.size, cartdetail.quantity, cartdetail.totalProduct
    ...
    public function selectQuantityProductInCart($idUser, $idProduct, $size)
    {
        $sql = "SELECT quantity
         FROM cartdetail JOIN carts ON cartdetail.idCart = carts.idCart
         WHERE idUser =$idUser AND idProduct =  $idProduct AND size = $size";
        return $this->db->selectAll($sql);
    }
