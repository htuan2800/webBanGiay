...
if (isset($_POST['add-cart'])) {
    ...
    $result = $cart->selectQuantityProductInCart($idUser, $idProduct, $size)->fetch_all(MYSQLI_ASSOC);
    // check nếu có sản phẩm size đó trong cart thì lấy ra số lượng không thì gán bằng 0
    $quantityInCart = (!$result) ? 0 : $result[0]["quantity"];

    // kiểm tra vượt số lượng bằng cách lấy số lượng ở input + với số hiện có trong cart ứng với từng size

    foreach ($product->selectSizeById($idProduct) as $item) {
        if ($item["size"] === $size) {
            if ($quantity + $quantityInCart <= $item["quantityRemain"]) {
