...
var dataCustomer = new Array(12).fill(0);
var dataCustomerNew = new Array(12).fill(0);

var data = new Promise(function (resolve, reject) {
	$.ajax({
		type: "POST",
		url: "./gui/statistics.php",
		data: {
			"getCustomer": true
		},
		dataType: "json",
...
data.then(function (response) {
	...
	for (var i = 0; i < response.customer.length; i++) {
		dataCustomer[response.customer[i]['MONTH'] - 1] = response.customer[i]['QUANTITY'];
	}

	for (var i = 0; i < response.customerNew.length; i++) {
		dataCustomerNew[response.customerNew[i]['MONTH'] - 1] = response.customerNew[i]['QUANTITY'];
	}
